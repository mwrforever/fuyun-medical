...
- 表外最小补充三处（PR 描述申报）：① fuyun-system pom 增 `spring-boot-starter-test`（test scope，BOM 托管）——SystemMasterDataPayloadTest 线格式冻结断言的必要测试依赖；② 新增 `StandardTelemetryMessageTest`（fuyun-common test）——CF-7 模型为冻结契约载体，补七字段语义与 JSON 往返断言（与 EventEnvelopeTest 同型，简报 §4 未列、按全局"测试与实现同提交"补齐）；③ 删除 fuyun-system api/.gitkeep（简报 §0-6：首个真实文件落地即删占位）。
