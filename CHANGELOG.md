...
- **宪法修订（先记再改）**：根 `AGENTS.md` §7 跨切约定新增「**回归红线出口（测试断言现代化流程）**」条目——既有
  ...
  原子性（断言修订+实现变更+回归锚同 PR）/ 留痕（CHANGELOG 本条目）。**2026-09-22 用户批复 D-21 选项①并批准
...
- **实现（DispenseWorkbenchView 发药签名 confirm）**：文案单号前置——`发药单 {dispenseNo} 签名后药品出库且不可逆，
  ...
  `cancelButtonText: '取消'`（R-3 缺口第三处闭合——英文 OK/Cancel 消尽；三处=PricingSettle/RefundApproval 批次 3
  两处 + 本处）。
