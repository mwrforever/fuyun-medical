...
- **回归红线出口（测试断言现代化流程，2026-09-22 用户批复 D-21 破例制度化）**：既有测试断言冻结的恰为「待改进的实现细节」而非业务行为时，禁止在功能 PR 内顺手改断言——须走**专项断言现代化 PR**，且同时满足四条边界：①**范围**——逐次批准、单点单次，不构成泛化先例（立项时在 PR 描述留痕批准出处）；②**严格度**——新断言不得低于原断言，须对新契约全量精确匹配（禁放宽为部分匹配或仅调用断言）；③**原子性**——断言修订 + 实现变更 + 回归锚同 PR 交付，禁混入其他功能；④**留痕**——决策记入 `CHANGELOG.md` 与本条目。（出口地址：凡「收尾动作被流程门槛卡住」类冲突——断言冻结、号段台账等——一律走显式出口 + 决策留痕，不得任由纪律从防回归滑向养欠账；**出口=既有合规路径，硬红线（安全 / 版本 / 编码红线、A.4.1-3 迁移禁改等）不在出口范围**——无合规出口者登记 `TASK.md` 待决策，不得经破例绕过。）
