...
### C.7 UI 设计思想 · 谋建琢三段律（前端必含·体系内置）

凡涉及 UI 界面的任何变更——新增页面 / 组件、样式 / 交互 / 布局调整与重构，**无论改动大小**——一律强制执行本三段律：当前 agent 必须先**实际加载**对应插件技能，再严格按「谋 → 建 → 琢」三段闭环依序推进。顺序不可逆、不得跳段、不得简化、不得偏离；禁止未加载技能凭自身经验「空思考」设计，禁止以「改动很小」为由绕过任何一段，违律产出一律不得交付。
...
────────────────────────────────────
 谋局定策 ｜ 前置思考 · 全局蓝图
────────────────────────────────────
 核心工具：@ui-ux-pro-max（强制加载）
 强制前置：先实际加载本技能正文，方可开始
           推演；未加载即思考 = 空思考，无效
 阶段职责：全局审视，敲定唯一最优方案
 · 通盘考量布局 / 样式 / 交互 / 动画四大维度
 · 持续思考、多轮推演、比较取舍
...
────────────────────────────────────
 施工依据：谋局阶段产出的设计方案
 强制前置：蓝图必须在位且产出自已加载
           @ui-ux-pro-max 的谋局段；施工全程
           保持该技能可查，无图一律退回谋局
 阶段职责：将既定方案完整转化为代码
 · 忠实执行设计决策，不偏移、不擅自降级
 · 结构清晰，为后续精修预留打磨空间
 · 以蓝图为唯一依据，不凭感觉发挥
 ▸ 红线：无图施工即违律；落地 ≠ 完成，粗成品严禁交付
```
...
────────────────────────────────────
 琢玉成器 ｜ 深度打磨 · 精修收口
────────────────────────────────────
 核心工具：@taste-skill（强制加载）
 强制前置：先实际加载本技能正文，方可打磨
           与验收；未加载即自行评判 = 空验收，
           无效
 阶段职责：对成品全面深度打磨，逼近极致
 · 逐层打磨组件 / 样式 / 交互 / 动画
 ...
 · 四维验收：高级视觉 / 高级交互 /
             流畅动画 / 高性能渲染
 ▸ 红线：未加载技能或四维标准缺一，视为未完成
```

> **协作纪律**：凡执行 UI 工作的当前 agent 与被派遣的 UI 相关 subagent，一律在开工前实际加载对应插件技能（谋 / 建 → `@ui-ux-pro-max`，琢 → `@taste-skill`）并逐段遵律执行；派遣指令必须明确写入该加载要求，未加载不得开工，凭经验空思考视为未执行本律。
