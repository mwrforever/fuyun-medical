...
/**
 * 机构变更事件载荷（system.org.changed，CF-2 首批主数据事件，M01 Spec §7 明示）。
 *
 * <p>触发场景：机构档案新增/变更/停用后广播，订阅方刷新本域机构缓存与数据权限边界。
 ...
 *
 * @param orgId      机构 ID（雪花 ID），非空；JSON 线格式经 Long→String 全局定制以字符串承载（A.3-8）
 * @param changeType 变更类型（如 CREATED/UPDATED/DEPRECATED），非空；占位取值随 PR-3 冻结
 */
public record OrgChangedPayload(Long orgId, String changeType) {}
