 ...
 * publish 为方法级事务——DRAFT 前置校验（非 DRAFT 抛 SYS-1013）→ 本版本置 PUBLISHED 并
 * 落 published_at/effective_at=now → 同类型其余 PUBLISHED 行置 DEPRECATED → 事务上下文内
 * 发布 Spring 应用事件（B.3-1 同步事件），由 SystemEventPublisher AFTER_COMMIT 监听在事务
 * 提交后才发 MQ——事务回滚则广播不出（A.4.2-7 事务内禁消息发送）。
 *
...
public class DictVersionServiceImpl extends ServiceImpl<DictVersionMapper, DictVersionEntity>
    ...
    public void publish(Long versionId) {
        ...
        // 2. 状态机前置校验：仅 DRAFT 可发布（SYS-1013/409，重复发布/废弃版本发布均拒绝）
        if (version.getStatus() != DictVersionStatus.DRAFT) {
            log.warn("字典版本发布被拒（状态不允许发布）：versionId={}，status={}", versionId, version.getStatus());
            throw new BizException(
                    SystemErrorCode.DICT_VERSION_NOT_PUBLISHABLE,
                    HttpStatus.CONFLICT,
                    "仅草稿版本的字典允许发布，当前状态：" + version.getStatus().getCode());
        }
        ...
        // 3. 本版本置 PUBLISHED，published_at/effective_at=now（P0 发布即生效，fy.delay 定时生效随 P1）
        OffsetDateTime now = OffsetDateTime.now();
        version.setStatus(DictVersionStatus.PUBLISHED);
        version.setPublishedAt(now);
        version.setEffectiveAt(now);
        this.updateById(version);
        // 4. 同类型旧 PUBLISHED 行置 DEPRECATED（同一 type 同一时刻仅一个 PUBLISHED）
        this.lambdaUpdate()
        ...
        // 5. 事务上下文内发布应用事件：AFTER_COMMIT 监听者于事务提交后才发 MQ（回滚则不广播）
