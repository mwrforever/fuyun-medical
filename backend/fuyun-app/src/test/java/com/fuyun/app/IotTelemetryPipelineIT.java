...
/**
 * IoT 遥测管道端到端集成测试骨架（PR-4 B4.2 交付步骤 1/2/4，BRIEF-PR4-01 §6.2）。
 *
 * <p>业务意图：以真实三中间件打通「AMQP 注入 → Qpid 消费 → 四路解析 → 绑定快照 → 批量冲突忽略
 * 落库 → 客户端确认」全链路——①种子设备档案与 BOUND 绑定（mapper 直插，快照值 9001/8001/1001）；
 * ②Qpid 生产端向 /queues/it.iot.telemetry 发 4 帧 CF-7 JSON（两两同 device+metric+occurredAt 重复
 * 键）→ 断言 iot_telemetry 恰 2 行且 patient_id/visit_id 为绑定快照值（唯一约束 ON CONFLICT DO
 * NOTHING 幂等与快照冗余的真实链路证据）；③毒丸隔离：发 1 帧非 JSON + 1 帧缺必填字段 → 断言
 ...
 *
 * <p>B4.3 扩展占位（本任务范围外，按 §6.2 串联）：步骤 3 = STOMP 遥测摘要断言（订阅
 * /topic/iot/telemetry/1001）；步骤 5 = 状态扇出全链（状态帧 → iot_device 更新 + fy.topic 自事件
 * 消费 + STOMP 设备状态主题）；步骤 6 = MQ 侧幂等重投回归；步骤 7 = HTTP 兜底 401/202 与同键去重。
 *
 * <p>三步断言按 @Order 串联（消费状态跨步累积属业务链路语义）。
 */
@Testcontainers
@SpringBootTest
@ActiveProfiles("test")
...
class IotTelemetryPipelineIT {
     ...
     */
    @Autowired
    IotTelemetryPipelineIT(JdbcTemplate jdbcTemplate, IotDeviceMapper deviceMapper, IotBindingMapper bindingMapper) {
