...
/**
 * 退费回执监听器单测（billing.refund.approved 消费侧，Task 6 + Task 10 order 分支）：合规七组件
 * 载荷解析双分支分发委托（appointment 分支先行、order 分支之后追加）与缺 settlementId 不合规帧
 * 显式拒绝（死信留痕）。业务体为包级 handle 方法，@RabbitListener 入口仅做 consume 委托（模板
 * 三段式已在 IT 面验证，BillingPharmacyOccupyListenerTest 同款形态）。
 */
...
class OutpatientRefundApprovedListenerTest {
    ...
    @Test
    @DisplayName("合规回执：解析七组件并双分支分发（appointment 终态先行、order 逆向扇出之后追加）")
    void handleDelegatesWellFormedReceiptToService() throws Exception {
        new OutpatientRefundApprovedListener(null, appointmentService, chargingService)
        ...
        verify(chargingService).onRefundApproved(any());
    }
    ...
    void rejectEnvelopeWithoutSettlement() throws Exception {
        assertThatThrownBy(() -> new OutpatientRefundApprovedListener(null, appointmentService, chargingService)
        ...
        verify(chargingService, never()).onRefundApproved(any());
    }
}
