...
/**
 * 退药受理回流消费侧（pharmacy.dispense.returned，V702 id 29 既有登记；「已退药」派生镜像唯一
 * 回流通道）：RX_REF 引用行镜像按 fullReturn 组件迁移（整单=FULL_RETURNED/部分=PART_RETURNED，
 * 单调不回退）业务分发面。载荷锚守卫归 IChargingService.onDispenseReturned，本监听器仅做编排所
 * 消费组件（rxNo/fullReturn/dispenseNo；lines 组件归 billing 占用回退面，本编排不消费不解析）
 * 解析与三段式委托。归 internal/，Bean 注册点 OutpatientMessagingConfig @Import。
 */
@Slf4j
public class OutpatientDispenseReturnedListener {
     ...
     * @param chargingService 收费编排服务，非空；退药回流业务体（镜像单调迁移）
     */
    public OutpatientDispenseReturnedListener(
    ...
    /**
     * 退药受理回流事件入口（标准三段式：NX 去重 → 业务 → PROCESSED 登记）。
     *
    ...
    public void onDispenseReturned(Message message) {
        consumerSupport.consume(message, this::handleDispenseReturned);
    }

    /**
     * 回流业务体（包级可见供单测直驱；@RabbitListener 入口仅做 consume 委托）：V702 id 29 载荷
     * 所消费组件解析后委托收费编排。
     *
     * @param envelope 已解码信封，非空
     */
    void handleDispenseReturned(EventEnvelope envelope) {
        JsonNode payload = envelope.payload();
        chargingService.onDispenseReturned(
                payload.path("rxNo").asText(""),
                payload.path("fullReturn").asBoolean(false),
