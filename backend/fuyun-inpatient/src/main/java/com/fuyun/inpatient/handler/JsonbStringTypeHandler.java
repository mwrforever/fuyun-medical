package com.fuyun.inpatient.handler;
...
/**
 * JSONB 文本列 TypeHandler（String ↔ jsonb，PG 专用；nursing 段 JsonbStringTypeHandler 同款
 * 先例——模块间禁依赖，本模块内复制承载）：MyBatis 默认对 String 参数以 varchar 类型发送，
 * PostgreSQL 强类型拒绝 varchar→jsonb 隐式转换（nursing Task 11 IT 实测暴露）。写侧以
 * {@link Types#OTHER} 发送——pgjdbc 以未指定类型参数承载，服务端按目标列 jsonb 上下文收参；
 * 读侧 getString 直读。经实体字段 {@code @TableField(typeHandler = ...)} 逐列挂载
 * （禁全局注册——String 全局接管会波及全部文本参数）。
 * 线程安全：无状态。
 */
...
public class JsonbStringTypeHandler extends BaseTypeHandler<String> {
    ...
    /**
     * 读侧：jsonb 列经 getString 直读（pgjdbc 返回 JSON 文本）。
     *
     * @param rs         结果集，非空
