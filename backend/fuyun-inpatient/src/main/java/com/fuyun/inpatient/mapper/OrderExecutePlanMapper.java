...
/**
 * 住院医嘱执行计划 mapper（V906 order_execute_plan）：单表链式能力 + 计划作废/重定向条件
 * 更新注解 SQL（GC26：条件更新一律 @Update + 影响行数判定，显式补 deleted=0；状态字面量
 * 与 V906 列值域、PlanStatus code 逐字同源）。执行回签 CAS（PENDING→EXECUTED）归 Task 8
 * W-33 回签端点落码，本批不含。
 */
@Mapper
