 ...
 * @param dosage       剂量（数值字符串如 0.5，≤32 对齐 V904 列宽），药品行必填（服务层拒 IP-1011）；来源：医生站录入
 * @param dosageUnit   剂量单位（如 g/ml），药品行必填（服务层拒 IP-1011）；来源：医生站录入
 * @param route        给药途径（M01 medication.route 字典 code，≤16 对齐 V904 列宽），药品行必填（服务层拒 IP-1011）；来源：医生站选择
 * @param dripRate     滴速（如 40 滴/分，≤32 对齐 V904 列宽），可空——静滴类医嘱携带；来源：医生站录入
...
public record OrderItemRequest(
        ...
        @Size(max = 32, message = "dosage 长度须 ≤32") String dosage,

        String dosageUnit,

        @Size(max = 16, message = "route 长度须 ≤16") String route,

        @Size(max = 32, message = "dripRate 长度须 ≤32") String dripRate,
