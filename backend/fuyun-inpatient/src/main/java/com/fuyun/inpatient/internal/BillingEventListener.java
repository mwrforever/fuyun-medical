 ...
 * （accountId/patientId/visitId/balance/status）的消费逻辑归 Task 10（欠费标识刷新——本类先
 * 挂监听消费入幂等三段式，消息不积压）。幂等两层：eventId 构件幂等（IdempotentConsumerSupport
 * 三段式）+ 业务级 CAS 限定幂等（IS NULL/BLOCKED 条件更新零行直返）。载荷以 JsonNode 读
 * （消费侧禁依赖 producer 载荷 record——PharmacyAuditReplyListener 同款口径）。归 internal/；
 * Bean 注册点 InpatientMessagingConfig @Import。
 */
@Slf4j
public class BillingEventListener {
    ...
    /**
     * 押金变动回执业务体（包级直驱可测）：载荷 accountId/patientId/visitId/balance/status
     * （V605 id 21 冻结契约）——欠费标识刷新（visit.arrears_flag）消费逻辑归 Task 10
     * （FU-M04-08），本面先承载幂等消费防消息积压。
     *
     * @param envelope 事件信封，非空；载荷契约 V605 id 21 冻结
     */
    void handleDepositChanged(EventEnvelope envelope) {
        // TODO(FU-M04-08): 欠费标识刷新消费逻辑（visit.arrears_flag 联动 billing 预警状态），
        // 计划于 P2 PR-1 Task 10 引入（本面已按幂等三段式消费，Task 10 仅补业务体）
        log.info("押金变动回执已消费（欠费标识刷新归 Task 10）：eventType={}，occurredAt={}", envelope.eventType(), envelope.occurredAt());
    }
