...
| D-8 | identifier.changed 发布点无事务的中继兜底策略（限定 Task 13 消化） | PR-2 Task 7（2026-09-17）实证：补挂端点按 controller 编排（attach → publishChanged → getById，controller 禁事务红线），publishChanged 发 Spring 应用事件时无活动事务——Task 13 中继若沿用 SystemEventPublisher 范式（@TransactionalEventListener(AFTER_COMMIT)，无 fallbackExecution），该事件将静默不出 MQ；二选一裁决：①中继 fallbackExecution=true 兜底（无事务也发，让渡「提交后才发」保证）②补挂编排下沉 service 写方法（@Transactional 内 publish，保持 AFTER_COMMIT 语义）；裁决后同步修正 IPatientIdentifierService.publishChanged javadoc 措辞 | Task 13 PatientEventPublisher、PatientIdentifierController.attach、IPatientIdentifierService.publishChanged javadoc |
