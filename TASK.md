...
| W-16 | billing 退费 execute 并发双发同退费单无幂等守卫 | 修复轮复审新发现（PR-3，2026-09-18）：execute 状态守卫为读值（RefundServiceImpl 判 APPROVED 后 record(REFUND)），绕 UI 并发双发可双贷记——与已收口三缺口同族 TOCTOU 但不在用户裁决三项范围；同族修法=execute 状态迁移改 CAS 抢 EXECUTED 锚（PR-4 顺手与 W-17 一并收口）；含 execute 读回侧卡行金额>0 与退款额≤卡侧原付合计加固（复审 F6） | fuyun-billing RefundServiceImpl.execute、refund_request 状态迁移 |
| W-17 | billing 退费审批路径在途申请不占 refundedFen 额度 | 修复轮复审新发现（PR-3，2026-09-18）：refundedFen 聚合仅 APPROVED/EXECUTED，PENDING_APPROVAL 不算——并发双审批类申请可双双进队列，双批后叠加 W-16 可双退；修复裁决口径明确限定免审路径。与 W-16 同批收口（额度口径含在途或审批串行化） | fuyun-billing RefundServiceImpl.apply/refundedFen 聚合 |
| W-18 | billing 退费 apply 缺结算单已结算/费用行归属与状态守卫 | 构造请求可对 DRAFT 结算单的费用行发起退费（免审即 APPROVED 并发事件、execute 后跳过 SETTLED 主链直置 REFUNDED）；前端 UI 有兜（status∈{SETTLED,PART_REFUND}+settlementId 匹配）但后端未兜；复审 fresh-eyes 新发现、非本轮引入；与 W-16/W-17 同族状态守卫缺口，PR-4 一并收口 | fuyun-billing RefundServiceImpl.apply、相关单测 |
