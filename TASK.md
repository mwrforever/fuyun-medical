...
| D-10 | 宪法 B.3-3 监听器注解包路径约定是否修宪明确 | PR-1a 执行实证（2026-09-14，PR #16）：`org.springframework.modulith.ApplicationModuleListener` 在 1.4.13 标记 @Deprecated(since="1.1", forRemoval=true)，官方指定替代为 `org.springframework.modulith.events.ApplicationModuleListener`（同名注解迁移至 spring-modulith-events-api，组合语义一致）；代码已用 events 包路径。宪法 B.3-3 现写短名——若用户裁决修宪，改动面=B.3-3 补一句「注解取 org.springframework.modulith.events 包路径（api 包同名注解已废弃 forRemoval）」；若不修，改动面=零（短名本就成立） | backend/AGENTS.md B.3-3 |
| D-11 | 宪法 C.4 命令清单是否补「单模块构建须 -am」提示 | PR-1a 真栈探针实证（2026-09-14，PR #16）：`mvn -pl fuyun-app` 不带 `-am` 时会从本地仓库取旧 fuyun-integration jar，致 Flyway 报「迁移缺失」或边界测试假违规（多模块反应堆中模块间依赖取安装版而非反应堆版）；修复=命令统一带 `-am`。若用户裁决修宪，改动面=C.4 命令清单加一条提示；若不修，改动面=零（属操作经验） | backend/AGENTS.md C.4 |
| D-12 | IoT 停机排空测试 flaky 处置 | CI 第四轮（PR #16）暴露 1 次：`IotAmqpTelemetryConsumerTest.drainsInFlightFramesOnGracefulStop` 断言「停机排空 2 帧」实收 1 帧——优雅停机与在途帧投递的时序竞态（本地全量 verify 三次全绿、同 PR 重跑绿，本分支未触碰 fuyun-iot，历史遗留非 PR-1a 引入）。待用户裁决：①修（排空等待改确定性同步，如 latch 计数对齐在途帧数）；②保持观察（记录 flake 频率再定）。若裁决修，改动面=该测试方法+可能的 ConsumerTestSupport | fuyun-iot IotAmqpTelemetryConsumerTest |
...
| 编号 | 事项 | 说明 |
| --- | --- | --- |
| W-4 | Flyway 迁移号段归属与版本唯一的 CI 自动校验 | 宪法 A.4.1-2 要求 CI 校验号段归属；PR-2 起号段登记生效（integration=V1–V99、患者 V100–V199、医嘱 V200–V299、系统建议 V300–V399、物联建议 V400–V499，V500 起先登记先占，载体 = 各 PR 简报 + CHANGELOG），校验脚本随 CI 完整化补建（BRIEF-PR2-01 §8-3 建议项）。**PR-1a 实证补充（2026-09-14）**：号段归属合法 ≠ 执行顺序合法——真库历史已应用 300/400 段后，integration 段新增 V6/V7（号段内合法）仍被 Flyway 判 out-of-order 拒绝（backend 启动失败，已改 V500/V501）；校验脚本须同时校验「新迁移版本号 > 全部 locations 已有最大版本号」 | CI hygiene/backend job 校验脚本 |
