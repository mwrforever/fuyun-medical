...
| D-20 | fy.delay 延迟档位 quorum 队列 TTL 到期帧零流量时滞留（PR-5 Task 12 真栈实证，2026-09-21） | RabbitMQ quorum 队列 x-message-ttl 到期依赖队头处理驱动的惰性过期而非独立定时扫除——空队列/低峰期过期帧不投递 DLX（Task 12 OutpatientRefundRollbackIT 45s 两形态探针零死信实证，TASK.md T-R3-4 压测项实测回填）；**忙碌流量下延迟至下一笔入队/投递触发，非完全断链**。三选项：①延迟队列单独豁免 classic 类型（牺牲 quorum 镜像语义换定时扫除）②向 fy.delay 周期投递 tick 触发队头过期（引入常驻消息面）③维持现状+Spec 注记 P2 修复+运维兜底（对账 sweep 检出滞留占位）。当前代码面：pay-hold 键 TTL=支付时限+5min 缓冲与 IT 合成信封承载不受影响；OutpatientRefundRollbackIT 注记与台账在案 | fuyun-app MessagingGovernance fy.delay 声明（declareDelayQueue 单档位 appointment-timeout）、outpatient 支付超时释放链（appointment.timeout→NO_SHOW→双面回池→信用限约）、TASK.md T-R3-4 |
| D-21 | 发药工作台「发药签名」confirm 弹窗两参调用被 spec 冻结断言锁死——R-3 中文按钮文案与 §4.4 单号回显未能落地（批次 4 主控裁决偏差，2026-09-22 收口登记） | `DispenseWorkbenchView` 发药签名 confirm 两参调用被其 spec `:198-201` `toHaveBeenCalledWith` 锁死文案（「发药签名后药品出库且不可逆，确认发药？」）与元数——R-3 confirmButtonText 第三实参（ElMessageBox 函数式挂载不继承 locale，渲染英文 OK/Cancel 缺口；三处已完成两处=PricingSettle/RefundApproval 批次 3）与设计文档 §4.4「发药单 ${dispenseNo} 单号回显」任一落地必破冻结断言；按「改实现不改断言」红线本调用不动（代码内中文注释登记缺口；台账登记行号 :149，Task 16 打磨后漂移至 :168）。选项：①专项 PR 修订该断言（新文案+三参）并补 R-3/回显两交付 ②维持现状 | web/apps/workstation/src/views/pharmacy/DispenseWorkbenchView.vue 发药签名 confirm 调用+其 .spec.ts 冻结断言 |
...
| W-23 | prescription_item.status 列注释失实订正迁移（W-22⑨ DB 残余面） | V701 内联注释称「status 由退药链回写」实为主代码零写入点（退场写 dispense_item.item_status、退药链仅回写 returned_quantity——代码侧 javadoc 已随 W-22 fix PR 订正）；宪法 A.4.1-3 禁改已应用迁移，订正须新迁移 `COMMENT ON COLUMN`；**号段必须用 pharmacy V706**——V704/V705 已被 PR-5 计划预定（Flyway 全局版本唯一+乱序守卫，先落 V706+ 会拦截 PR-5 预定号段），须待 PR-5 合入后落（随 PR-5 顺手项或后续 fix PR，闭合时删除本行） | fuyun-pharmacy V706 迁移（仅 COMMENT ON COLUMN pharmacy.prescription_item.status，头部注明订正缘由）、CHANGELOG 号段登记 |
| W-24 | getByRxNo 同 rxNo 取消态多行 selectOne 抛 500（W-22 质量门复核裁定登记） | DispenseServiceImpl.getByRxNo 的 selectOne 未排除取消态——uk_dispense_rx_active 部分唯一索引（WHERE status<>'CANCELLED'）许可「CANCELLED 行+重建活动行」共存，届时 selectOne 多行抛错兜底 500；今日无整单 CANCELLED 写入点不可达（base 既有，W-22 fix PR 零触碰），但 PR-5 Task 11 作废路径将引入整单取消态写入点、隐患恰转可达且在热点读上；修法与 occupancy 批查同款一行（`.ne(status,"CANCELLED")`），随 PR-5 作废路径改造或首个引入整单取消态的 PR 顺手闭合（闭合时删除本行）；**代码修复已随 PR-5 交付（Task 11：`.ne(status,"CANCELLED")`+谓词用例），行删除待合并后执行** | backend/fuyun-pharmacy service/impl/DispenseServiceImpl.getByRxNo、相关单测断言 |
