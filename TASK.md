...
| W-16 | billing 退费 execute 并发双发同退费单无幂等守卫 | 修复轮复审新发现（PR-3，2026-09-18）：execute 状态守卫为读值（RefundServiceImpl 判 APPROVED 后 record(REFUND)），绕 UI 并发双发可双贷记——与已收口三缺口同族 TOCTOU 但不在用户裁决三项范围；同族修法=execute 状态迁移改 CAS 抢 EXECUTED 锚（PR-4 顺手与 W-17 一并收口） | fuyun-billing RefundServiceImpl.execute、refund_request 状态迁移 |
