...
| D-13 | 一卡通 close/freeze 全量回写并发窗口（M13 接线前收口） | PR-2 Task 9 修复轮次（2026-09-17 审查 Important 1）：CardAccountServiceImpl 的 close/freeze 为「getById 读出全实体 → updateById 全量回写」模式，MP 默认 NOT_NULL 字段策略会把读出的 balance 等非空列一并 SET——若未来 M13 经 CardAccountLedger.record() 的并发记账在读与写之间提交，销户会把余额回写旧值（CLOSED 账户资金蒸发、balance_after 对账锚点失衡）。当前未活化：record() 仅 api 接口位、M13 前无调用方（拍板 5）。裁决方向（M13 接线前二选一收口）：①销户改条件更新判影响行数（UPDATE ... SET status='CLOSED' WHERE id=? AND status IN('ACTIVE','FROZEN') AND balance=0）②改最小实体回写（仅 id/status/closedAt 三字段）避免余额回写 | CardAccountServiceImpl close/freeze、M13 CardAccountLedger 调用方、PR-3 对账 |
| D-14 | 就诊卡 LOST 找回路径缺定义（bind 守卫收口后挂失卡找回无合法转移） | PR-2 Task 10 修复轮次（2026-09-17 审查 Important 2）：bind 收口为仅无主卡（patientId 空/零值）可绑定后，LOST→ACTIVE 转移被阻断——挂失卡失而复得时无合法状态转移路径，M02 Spec §7 动作端点仅定义 issue/bind/loss/replace/unbind 未定义「找回」。裁决方向二选一：①后续 Spec 增补「找回」动作端点（LOST→ACTIVE + identifier.changed）②保留人工路径（窗口解绑注销旧卡后发新卡，或按 replace 补卡语义办理） | VisitCardServiceImpl.bind、M02 Spec §7、docs/specs 对应模块 Spec 同步 |
| D-15 | 健康档案 onsetDate 非法 ISO 文本走 500（契约缺口） | PR-2 Task 11 修复轮次（2026-09-16 审查 Important 1）：HealthItemCreateRequest/HealthItemCorrectRequest 的 onsetDate 契约为无校验注解的 String，HealthSummaryServiceImpl.parseOnsetDate 以 LocalDate.parse 直解——用户输入非法 ISO 文本（如 2026-13-40 或非日期串）抛 DateTimeParseException 走全局 500，违反「用户输入错误应 ProblemDetail + PAT-xxxx」；正则近似校验兜不住「合法格式非法日期」（如 2026-02-30）。裁决方向：①新 PAT 错误码 + ISO 形状校验注解/显式解析转 400 ②宽容解析策略；裁决后同步 M02 Spec §7 与 docs/specs（改接口契约必同步） | HealthItemCreateRequest/HealthItemCorrectRequest 的 onsetDate、HealthSummaryServiceImpl.parseOnsetDate、M02 Spec §7、PrivacyController 的 signedAtIso/validToIso（OffsetDateTime.parse 无守卫，非法 ISO 文本走 500，与 onsetDate 同型） |
| D-16 | 授权三态硬门禁挂接 unmask 的归属与口径 | PR-2 Task 12 修复轮次（2026-09-16 审查 Important 1）：当前 unmask 仅角色豁免单门禁（PrivacyServiceImpl unmask 三步），privacy_auth REVOKED/EXPIRED/无授权不拒绝明文查阅，等保红线「三态拒绝」在本 PR 不闭环（诊疗关系校验先行豁免随 PR 描述登记）。裁决方向：归属 M03/M04 注册后实时校验 or 保留角色豁免单门禁并由后续 PR 补硬门禁 | PrivacyServiceImpl.unmask、M03/M04 消费方、M02 Spec FU-M02-06 |
| D-17 | 同证件不同姓名建档的 ID_CARD_CONFLICT 路径不可达 | PR-2 Task 15 修复轮次（2026-09-16 审查 Important 1）：V100 `uk_patient_identifier_type_hash (identifier_type, value_hash) WHERE deleted=0` 部分唯一索引 + 挂接事务边界——register() 第④步 ID_CARD 恒挂接，同证件建档必撞唯一索引 → PAT-1002 整体回滚，SUSPECT 强冲突（ID_CARD_CONFLICT）待审行永不存在，与 M02 Spec ID_CARD_CONFLICT 能力描述冲突（真栈 IT 实证，PR-2 Task 15，2026-09-16）。裁决方向：①Spec 删除/改写 ID_CARD_CONFLICT 建档场景为「证件冲突直接拒建」②建档改「同证件异名先落疑似对再挂标识」两步事务 | PatientRegistrationServiceImpl.register、M02 Spec FU-M02-01/02、匹配引擎强标识语义 |
| D-18 | openapi 生成契约 int64 字段类型（number）与后端 Long→String 运行时输出漂移 | PR-2 Task 17 修复轮次（2026-09-16 审查 Important 1）：openapi-typescript 7.13.0 按 OpenAPI `format: int64` 生成 number（api.d.ts 的 PatientVO.patientId、PatientMatchCheckVO.candidatePatientId、PageResult*.total 等），与后端 Jackson 全局 Long→String 运行时输出字符串、web 宪法 A.3-6「Long 一律 string 承载」红线漂移。当前缓解：workstation 患者三页统一 String() 兜底消费，无 ===/Set/Map/算术危险消费点（Task 17 审查逐点核实，可安全交付）。裁决方向：①`gen:api` 加 transform/mapper（int64→string）②后端 springdoc 将 Long 全局序列化为 string schema——两者皆属机制修订，须先记 CHANGELOG 再改 + 前端五连复验；另将 PageResult.total 跨层契约（packages/shared/src/index.ts 手写声明 string）与生成 VO 标注 number 不同源问题一并根治 | packages/shared/src/api.d.ts 全站 int64 字段、workstation api/patient.ts 检索返回内联类型、后续所有 api 模块生成类型消费 |
