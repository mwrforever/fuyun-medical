...
| W-22 | PR-4 审查遗留项归档（/code-review 低于阈值条目与收尾观察，2026-09-19） | PR-4 /code-review 置信评分 <80 被插件规则过滤的合规项+收尾观察，逐项核实为真但不阻塞合并，归后续 PR 处置：①PharmacyMasterDataCache Redis 键无 fy: 前缀且无 TTL（违 A.5-1 双重条款）；②pharmacy enums 15 枚举主代码零引用、状态字面量散落 impl（A.2-6/7——brief 冻结 String 形态，宜随状态机枚举化一并改造）；③PrescriptionServiceImpl.list 与 occupancy 循环单查 N+1（A.4.3-14）；④处方/发药明细循环逐条 insert 宜 saveBatch（A.4.3-16）；⑤DispenseController:96 全限定类名（A.1-13）；⑥pharmacy 前端动作按钮缺在途防抖（PR #26 B4 先例：loading+守卫+零出网用例）；⑦quantity/returnQuantity/splitRatio 裸 parse 非数字入参 500 出契约（PR #26 B2「禁裸 parse」先例，5 处）；⑧PrescriptionServiceImpl:139 outbox 投递保证过度承诺注释（实为 AFTER_COMMIT 直发，outbox 属 P1）与 PharmacyBillingSyncListener:14 前瞻注释落空——两处注释订正；⑨prescription_item.status 列注释称由退药链回写但主代码无写入点（V701 列注释或退药链二选一订正）。处置时机：PR-5 开工前 fix PR 或随 PR-5 顺手项（闭合时删除本行） | fuyun-pharmacy 全域、web/apps/workstation pharmacy 三页、backend/fuyun-pharmacy V701 |
